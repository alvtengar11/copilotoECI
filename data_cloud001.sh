...
echo "Carga automática de productos en cloud001"
...
DYNADMINCLIENT_CONFIG="${DYNADMINCLIENT_HOME_DIR}/config/dynadminclient-omnistore-uat001.json"
...
REMOTE_HOST="preprod1@node1.uat001.azure.cloud.elcorteingles.es"
REMOTE_BBDD="(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(Host=db.uat001.azure.cloud.elcorteingles.es)(Port=1524))(CONNECT_DATA=(SERVICE_NAME=worker)))"
...
ejecutaCatalogo(){
	...
	#Ejecutar catálogo
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y -t uat001_h_loader -b "launch ${CATALOGO}"
	while ${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y --quiet -t uat001_h_loader -b "status ${CATALOGO}"| grep '"deploymentStatus": "IN_PROGRESS"'; 
	do
		sleep 20
	done
}
...
ejecutaFlatteners(){
	...
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y -t uat001_h_loader -b "launch DISTRIBUTED_CONTENT_GROUPS_FLATTENER"
	while ${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y --quiet -t uat001_h_loader -b "status DISTRIBUTED_CONTENT_GROUPS_FLATTENER"| grep '"deploymentStatus": "IN_PROGRESS"'; 
	do
		sleep 20
	done

	#Ejecutar Flatterer Promo
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y -t uat001_h_loader -b "launch DISTRIBUTED_PROMOTION_FLATTENER"
	while ${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y --quiet -t uat001_h_loader -b "status DISTRIBUTED_PROMOTION_FLATTENER"| grep '"deploymentStatus": "IN_PROGRESS"'; 
	do
		sleep 20
	done
}
...
ejecutaIndexacionPrincipal(){
	...
	#Ejecutar indexación
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -y -f ${DYNADMINCLIENT_CONFIG} -t uat001_h_index1 -b "launch ${INDEX}"
	while ${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y --quiet -t uat001_h_index1 -b "status ${INDEX}"| grep '"deploymentStatus": "IN_PROGRESS"'; 
	do
		sleep 20
	done
...
ejecutaOtrasIndexaciones(){
	...
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -y -f ${DYNADMINCLIENT_CONFIG} -t uat001_h_index1 -b "launch INDEX_BASELINE_SEARCH_HIPERCOR"
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -y -f ${DYNADMINCLIENT_CONFIG} -t uat001_h_index1 -b "launch INDEX_BASELINE_SEARCH_LTC"
	while ${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y --quiet -t uat001_h_index1 -b "status INDEX_BASELINE_SEARCH_LTC"| grep '"deploymentStatus": "IN_PROGRESS"'; 
	do
		sleep 20
	done
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -y -f ${DYNADMINCLIENT_CONFIG} -t uat001_h_index1 -b "launch INDEX_BASELINE_SEARCH_SUPER"
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -y -f ${DYNADMINCLIENT_CONFIG} -t uat001_h_index1 -b "launch INDEX_BASELINE_SEARCH_PORTUGAL"
	while ${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y --quiet -t uat001_h_index1 -b "status INDEX_BASELINE_SEARCH_PORTUGAL"| grep '"deploymentStatus": "IN_PROGRESS"'; 
	do
		sleep 20
	done
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -y -f ${DYNADMINCLIENT_CONFIG} -t uat001_h_index1 -b "launch INDEX_BASELINE_SEARCH_CATERING"
	${DYNADMINCLIENT_HOME}/dynadminclient.sh -y -f ${DYNADMINCLIENT_CONFIG} -t uat001_h_index1 -b "launch INDEX_BASELINE_SEARCH_ECI_UNIFORMS"
	while ${DYNADMINCLIENT_HOME}/dynadminclient.sh -f ${DYNADMINCLIENT_CONFIG} -y --quiet -t uat001_h_index1 -b "status INDEX_BASELINE_SEARCH_ECI_UNIFORMS"| grep '"deploymentStatus": "IN_PROGRESS"'; 
	do
		sleep 20
	done
}
